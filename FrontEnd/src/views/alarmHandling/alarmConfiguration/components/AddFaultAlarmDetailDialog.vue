        ...
        <el-form-item label="提醒方式">
          <el-checkbox-group v-model="formData.notices" class="w-full flex">
            <div class="items-center flex">
              ...
              <el-select
                ref="selRef"
                v-model="formData.webNoticeUsernames"
                :disabled="formData.notices.indexOf(1) === -1"
                multiple
                placeholder="请选择用户"
                clearable
                ...
                style="width: 220px"
              >
                <el-option v-for="item in userList" :key="item.username" :label="item.realName" :value="item.username"></el-option>
              </el-select>
            </div>
    ...
    <section class="flex bot-section h-[550px]">
      <div class="bot-left w-[300px]">
        <div class="w-full flex items-center mt-[20px]">
          ...
          <el-select v-model="formData.thingModels[0].thingModelId" style="width: 200px" @change="deviceTypeChange" clearable>
      ...
      <div class="w-[1300px] overflow-auto">
        <el-form ref="queryForm" :inline="true" :model="queryFormData" class="flex flex-wrap mt-[10px] ml-[60px]">
          ...
          <el-form-item label="所属系统">
            <el-select v-model="queryFormData.equipmentSystem" style="width: 200px">
              <el-option v-for="item in systemList" :key="item.itemValue" :label="item.itemText" :value="item.itemValue" />
            </el-select>
          </el-form-item>
          <el-form-item label="所在建筑">
            <el-select v-model="queryFormData.building" style="width: 200px">
              <el-option v-for="item in buildingList" :key="item.itemValue" :label="item.itemText" :value="item.itemValue" />
            </el-select>
          </el-form-item>
          <el-form-item label="所在楼层">
            <el-select v-model="queryFormData.floor" style="width: 200px">
              <el-option v-for="item in floorsList" :key="item.itemValue" :label="item.itemText" :value="item.itemValue" />
            </el-select>
          </el-form-item>
          <el-form-item label="所在区域">
            <el-select v-model="queryFormData.equipmentLocation" style="width: 200px">
              <el-option v-for="item in areaList" :key="item.itemValue" :label="item.itemText" :value="item.itemValue" />
            </el-select>
          </el-form-item>
        ...
        <el-table
          ref="tableRef"
          :data="tableData"
          class="mb-16px customTable mt-16px"
...
import {
  getAlarmLevelList,
  getBaseUserList,
  getAlarmEquipmentThingModels,
  getAlarmEquipmentEquipments,
  alarmStrategyAdd,
  getAlarmStrategyById,
  alarmStrategyUpdate,
} from "../../ts/index";
...
const formData = ref<any>({
  ...
  filterTimeUnit: "",
  rules: [],
  thingModels: [{ thingModelId: "", thingModelPropertyIds: [] }],
  equipmentIds: [],
});
...
const handleSelectionChangeAll = (val) => {
  if (val.length == 0) {
    ...
    const selectionIds = new Set(selectionData.value.map((x) => x.equipmentId));
    formData.value.equipmentIds = formData.value.equipmentIds.filter((e) => !selectionIds.has(e));
  } else {
    const equipmentIdsSet = new Set(formData.value.equipmentIds);
    val.forEach((x) => {
      equipmentIdsSet.add(x.equipmentId);
    });
    formData.value.equipmentIds = Array.from(equipmentIdsSet);
    formData.value.equipmentIds = [...formData.value.equipmentIds];
  }
};
const handleSelectionChange = (val) => {
  if (val.length > 0) {
    ...
    // formData.value.equipmentIds = [...selection.value.concat(formData.value.equipmentIds)]
    val.forEach((item) => {
      if (!formData.value.equipmentIds.includes(item.equipmentId)) formData.value.equipmentIds.push(item.equipmentId);
    });
  }
};
const onTableSelect = (rows, row) => {
  const selected = rows.length && rows.includes(row);
  if (!selected) {
    const index = formData.value.equipmentIds.indexOf(row.equipmentId);
    selectionData.value.splice(index, 1); // 取消勾选，则删除数据
    formData.value.equipmentIds.splice(index, 1);
  } else {
    formData.value.equipmentIds.push(row.equipmentId);
    selectionData.value.push(row);
  }
};
const query = async () => {
  queryFormData.thingModelId = formData.value.thingModels[0].thingModelId;
  await getAlarmEquipmentEquipments(queryFormData, pageParam.value).then((res: any) => {
  ...
  const equipmentIdsSet = cloneDeep(new Set(formData.value.equipmentIds));
  const selectedRows = tableData.value.filter((v) => equipmentIdsSet.has(v.equipmentId));
  // selectedRows.forEach(row => tableRef.value?.toggleRowSelection(row, true));
  selectedRows.forEach((row) => tableRef.value?.toggleRowSelection(row, true));
};
const reset = () => {
...
const deviceTypeChange = (val) => {
  selection.value = [];
  selectionData.value = [];
  // formData.thingModels[0].thingModelId
  if (flag.value === false) {
    // formData.value.thingModels = [{ thingModelId: "", thingModelPropertyIds: [] }];
...
defineExpose({
  open: async (id) => {
    ...
    getAlarmEquipmentThingModels({ alarmType: 1 }).then((res: any) => {
      deviceTypeList.value = res.data;
    });
    // 查字典
    getDict("equipment_system").then((res: any) => {
      if (res.code === 200 && res.data.length > 0) systemList.value = res.data;
    });
    getDict("equipment_building").then((res: any) => {
      if (res.code === 200 && res.data.length > 0) buildingList.value = res.data;
    });
    getDict("equipment_floor").then((res: any) => {
      if (res.code === 200 && res.data.length > 0) floorsList.value = res.data;
    });
    getDict("equipment_area").then((res: any) => {
      if (res.code === 200 && res.data.length > 0) areaList.value = res.data;
    });
    if (id) {
      dialogTitle.value = "编辑故障告警";
      getAlarmStrategyById(id).then(async (res: any) => {
        formData.value = res.data;

        setTimeout(() => {
          deviceTypeChange(res.data.thingModels[0].thingModelId);
          // formData.value.thingModels = [{ thingModelId: res.data.thingModels[0].thingModelId, thingModelPropertyIds: res.data.thingModels[0].thingModelPropertyIds }]
