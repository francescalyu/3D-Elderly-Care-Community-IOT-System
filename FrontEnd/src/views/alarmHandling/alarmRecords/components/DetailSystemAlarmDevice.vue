...
<template>
  <el-dialog v-model="dialogVisible" title="" width="600px" :draggable="true" :close-on-click-modal="false"
    @close="handleClose">
    <template #header>
      <span class="el-dialog__title">通讯设备告警--查看</span>
    </template>
    <section class="content flex justify-between">
      <div class="left w-[300px]">
        ...
        <div>
          <span class="w-[80px]">远端IP：</span>
          <span>{{ formData.remoteIp }}</span>
        </div>
        <div>
        ...
        <div class="w-[550px]">
          ...
          <span style="overflow:visible;white-space: normal;overflow-wrap: break-word;width:400px">{{ formData.alarmDetail }}</span>
        </div>
      </div>
      <div class="right w-[300px]">
        ...
        <div>
          <span class="w-[80px]">类型：</span>
          <span>{{ formData.type }}</span>
        </div>
        <div>
          <span class="w-[80px]">远端端口：</span>
          <span>{{ formData.remotePort }}</span>
        </div>
        <div>
...
const formData = ref<any>({
  ...
  alarmTypeStr: '系统通讯设备告警',
  alarmDetail: 'KT001通讯设备告警',
  alarmLevelName: '严重',
  type: 'MODBUS_TCP',
  port: '',
  alarmTime: '2024.1.1 12:00:00'
})

const submit = async () => {
  emits('toDealAlarm', 6,curId.value)
