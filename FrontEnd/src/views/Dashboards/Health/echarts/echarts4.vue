<template>
  <div class="ec1_item" id="ecHealth4"></div>
</template>
...
const init = () => {
  ...
  const option9 = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        var text =
          "<span style='color:rgba(164, 204, 69, 1)'>" + params[0].axisValueLabel + "</span></br>";
        console.log(params);
        for (let i = 0; i < params.length; i++) {
          if (params[i].seriesName != "背景") {
            text += params[i].seriesName + ": " + params[i].value + " 小时 </br>";
          }
        }
        return text;
      },
  ...
  //   ];
  // }
  const myChart = echarts.init(document.getElementById("ecHealth4"));
  ...
  resizeObserver.observe(document.getElementById("ecHealth4"));
};
watch(
  () => [props.dataItem],
  () => {
