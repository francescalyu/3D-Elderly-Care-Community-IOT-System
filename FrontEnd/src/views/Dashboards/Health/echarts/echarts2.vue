<template>
  <div class="ec1_item" id="ecHealth2"></div>
</template>
...
const init = () => {
  ...
  const option9 = {
    ...
    legend: {
      ...
      formatter: function (val) {
        // console.log("val: ", val);
        return "平均" + val + " " + (sum || 0) + "次/分   Max 120  Min 98";
      },
    },
    xAxis: {
      name: "",
  ...
  const myChart = echarts.init(document.getElementById("ecHealth2"));
  ...
  resizeObserver.observe(document.getElementById("ecHealth2"));
};
watch(
  () => [props.dataItem],
  () => {
